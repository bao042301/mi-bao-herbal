...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* 題目字體 */
    .stRadio > label p { font-size: 1.2rem !important; font-weight: bold !important; margin-bottom: 15px !important; }

    /* 【優化】選項卡片：隱藏黑點、間距加大到 25px 更有呼吸感 */
    div[data-baseweb="radio"] input { display: none !important; }
    div[data-baseweb="radio"] div {
        background-color: #FFFFFF !important; border: 1px solid #E9EDC9 !important;
        border-radius: 15px !important; padding: 12px 18px !important; 
        margin-bottom: 25px !important; /* 從 18px 增加到 25px */
        transition: all 0.3s ease !important;
    }
    div[aria-checked="true"] { background-color: #7A8450 !important; border-color: #7A8450 !important; }
    div[aria-checked="true"] * { color: #FFFFFF !important; }

    /* 輸入框樣式 */
    .stTextInput input { background-color: #FFFFFF !important; border: 2px solid #E9EDC9 !important; border-radius: 12px !important; color: #4A4E31 !important; }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("當妳靜下心，妳的身體正低聲說著...？", 
                  ["我有些疲累，渴望溫潤透亮的晨光...", "我有些沉重，想找回輕盈自在的微風...", "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v19_q1")
    if st.button("緩緩走向下個瞬間 ➔"): # 豐富性調整 1
    ...
    q2 = st.radio("關於這段日子的作息，妳目前的感受是？", 
                  ["長時間待在冷氣房，循環緩慢且手腳冰冷", "工作忙碌常熬夜，晚餐不規律導致不適", "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v19_q2")
    if st.button("傾聽日常的節奏 ➔"): # 豐富性調整 2
    ...
    q3 = st.radio("在最需要喘息的午後，妳最嚮往什麼樣的瞬間？", 
                  ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v19_q3")
    ...
    st.write("### 💎 妳是米寶的新朋友嗎？")
    choice = st.radio("這是妳第一次預約米寶的「月度質感陪伴」嗎？", ["是的，我是新朋友", "不是，我是老朋友了（已有隨行杯）"], index=None, key="v19_choice")
    ...
    if "晨光" in ans[0]:
        diag, m_tea, a_tea = "暖陽系", "黃耆元氣茶 (14入) + 金菊牛蒡茶 (6入)", "當歸紅棗茶 (12入) + 黑豆漢方茶 (8入)"
    elif "微風" in ans[0]:
        diag, m_tea, a_tea = "微風系", "洛神山楂茶 (12入) + 金菊牛蒡茶 (8入)", "玫瑰決明茶 (10入) + 黑豆漢方茶 (10入)"
    else:
        diag, m_tea, a_tea = "清泉系", "金菊牛蒡茶 (15入) + 黃耆元氣茶 (5入)", "玫瑰決明茶 (14入) + 當歸紅棗茶 (6入)"
    ...
    <div class="result-card">
        <h3 style='text-align:center; font-size: 1.6rem;'>✨ 妳是：{diag}女子</h3>
        ...
        <p style='font-size:1rem; line-height:1.6;'>
            <b>本月陪伴清單包含：</b><br>
            • <b>40 入</b> 深度節律漢方茶組 (全月份份份量)<br>
            • 米寶生活風格手札 (紀錄妳的質感日常)<br>
            {gift_detail}
        </p>
        <p style='text-align:right; color:#7A8450; font-size: 1.1rem; font-weight: bold; margin-top:10px;'>
            月度質感陪伴價 $1,980
        </p>
    </div>
    """, unsafe_allow_html=True)
