...
# 1. 視覺全鎖定：V74 寬鬆比例、選中加粗放大、亮色模式防禦
...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    [data-testid="stRadio"] label {
        ...
        text-align: left !important; font-size: 0.95rem !important; line-height: 1.4 !important;
    }
...
if st.session_state.step <= 3:
    ...
    choice = st.radio("", ["是的，我是新朋友 👋", "我是老朋友 🤗"], index=None, key="id_v74", label_visibility="collapsed")
    ...
    p_choice = st.radio("", ["月度植感陪伴 (40入) $1,680", "一週輕體驗組 (10入) $680"], index=None, key="p_v74", label_visibility="collapsed")
    if st.button("查看您的專屬配比 ➔"):
        if p_choice:
            st.session_state.plan = p_choice
            if "新朋友" in st.session_state.is_first_time and "$1,680" in p_choice:
                st.session_state.step = 5
            else:
                st.session_state.custom_name = "熟客續購禮遇" if "$1,680" in p_choice else "植感體驗方案"
    ...
    st.markdown("<div style='background-color: #FFFFFF; padding: 15px; border-radius: 12px; border: 1px solid #E9EDC9; font-size:0.95rem; text-align:center;'>為您準備一只質感的玻璃隨行杯。<br>木蓋上將鐫刻您的專屬風格。</div>", unsafe_allow_html=True)
    u_name = st.text_input("雷刻文字 (最多12字)", max_chars=12, placeholder="例如：Mila")
    if st.button("查看您的專屬配方 ➔"):
        if u_name:
            st.session_state.custom_name = u_name
            st.session_state.step = 6
            st.rerun()
    ...
    # 使用原生的 line:// 協議，在 LINE 內跳轉最穩
    st.markdown(f'''<a href="line://ti/p/@716osfvq" target="_top" style="text-decoration:none;"><div style="background-color: #06C755; color: white; text-align: center; padding: 14px; border-radius: 15px; font-weight: bold; font-size: 1rem;">🌿 前往 LINE@ 貼上專屬配方 ➔</div></a>''', unsafe_allow_html=True)
    if st.button("重新探索"):
