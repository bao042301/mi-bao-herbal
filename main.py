...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    .quote { font-style: italic; color: #8B8B7A !important; text-align: center; margin-bottom: 15px; font-size: 0.85rem; }
    ...
    [data-testid="stRadio"] div[aria-checked="true"] label {
        ...
        box-shadow: 0 4px 12px rgba(122, 132, 80, 0.1) !important;
    }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光...", "我有些沉重，想找回輕盈自在的微風...", "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v34_q1")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，循環緩慢且手腳冰冷", "工作忙碌常熬夜，晚餐不規律導致不適", "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v34_q2")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v34_q3")
    ...
    choice = st.radio("", opts, index=None, key="v34_choice")
    if st.button("前往專屬的陪伴 ➔"):
        if choice == opts[0]:
            st.session_state.is_first_time = "是的"; st.session_state.step = 5; st.rerun()
        elif choice == opts[1]:
            st.session_state.is_first_time = "不是"; st.session_state.custom_name = "老朋友回購驚喜"; st.session_state.step = 6; st.rerun()
        else:
            st.warning("請選擇妳的身分喔")

elif st.session_state.step == 5:
    st.write("### 💎 鐫刻妳的專屬溫柔")
    st.markdown("<div style='background-color: #FFFFFF; padding: 15px; border-radius: 12px; border: 1px solid #E9EDC9; font-size:0.85rem; text-align:center;'>首購禮遇：為妳在木蓋上，鐫刻專屬的名字。</div>", unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background-color: #FFFFFF; padding: 20px; border-radius: 20px; border: 1px solid #E9EDC9;">
