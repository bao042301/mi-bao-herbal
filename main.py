...
# 1. 究極視覺鎖定 (選中底色暴力覆蓋、價格置中放大、複製按鈕強顯、選項靠左)
...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* 2. 【核心突破】選中後的底圖顏色強制覆蓋 */
    /* 使用三重屬性鎖定與同級選擇器，並放在 nth-of-type 之後以確保覆蓋 */
    ...
    [data-testid="stRadio"] input:checked + label {
        background-color: #E9EDC9 !important; /* 選定後轉為橄欖綠 */
        border: 2px solid #7A8450 !important;
        font-weight: bold !important;
        box-shadow: 0 2px 10px rgba(122, 132, 80, 0.2) !important;
    }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光", "我有些沉重，想找回輕盈自在的微風", "我有些燥熱，想念山間清徹甘甜的泉水"], index=None, key="v70_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，手腳冰冷循環差", "生活忙碌常熬夜，作息不規律隨意吃", "壓力大節奏快，心神緊繃難入眠"], index=None, key="v70_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，從容自在"], index=None, key="v70_q3", label_visibility="collapsed")
    ...
    opts = ["是的，我是新朋友", "我是老朋友"]
    choice = st.radio("", opts, index=None, key="v70_choice", label_visibility="collapsed")
