...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光", "我有些沉重，想找回輕盈自在的微風", "我有些燥熱，想念山間清澈甘甜的泉水"], index=None, key="q1", label_visibility="collapsed")
...
# ----------------- Step 4：一頁式方案整合 -----------------
    ...
    if st.button("查看我的專屬配比 ➔"):
        if selected:
            st.session_state.bundle = selected # 這裡會存入全名含價格
...
# ----------------- Step 5：結果呈現 (換行優化) -----------------
    ...
    if price_val >= 1880:
        if "晨光" in ans[0]: m_t, a_t = "黃耆元氣茶 (14入)<br>+ 金菊牛蒡茶 (6入)", "當歸紅棗茶 (12入)<br>+ 黑豆漢方茶 (8入)"
        elif "微風" in ans[0]: m_t, a_t = "洛神山楂茶 (12入)<br>+ 金菊牛蒡茶 (8入)", "玫瑰決明茶 (10入)<br>+ 黑豆漢方茶 (10入)"
        else: m_t, a_t = "金菊牛蒡茶 (15入)<br>+ 黃耆元氣茶 (5入)", "玫瑰決明茶 (14入)<br>+ 當歸紅棗茶 (6入)"
        gift = f"• 首購禮：專屬刻名隨行杯 ({name})" if price_val == 1980 else "• 老友禮：加贈驚喜茶包 3 入"
        amt = "40 入深度植感漢方茶組"
    else:
        if "晨光" in ans[0]: m_t, a_t = "黃耆元氣茶 (4入)<br>+ 金菊牛蒡茶 (1入)", "當歸紅棗茶 (3入)<br>+ 黑豆漢方茶 (2入)"
        elif "微風" in ans[0]: m_t, a_t = "洛神山楂茶 (3入)<br>+ 金菊牛蒡茶 (2入)", "玫瑰決明茶 (3入)<br>+ 黑豆漢方茶 (2入)"
...
# ----------------- Step 6：預約轉化 (訊息全名+換行) -----------------
    ...
    if price_val >= 1880:
        m_v, a_v = ("黃耆元氣茶(14)\n+ 金菊牛蒡茶(6)", "當歸紅棗茶(12)\n+ 黑豆漢方茶(8)") if "晨光" in ans[0] else ("洛神山楂茶(12)\n+ 金菊牛蒡茶(8)", "玫瑰決明茶(10)\n+ 黑豆漢方茶(10)") if "微風" in ans[0] else ("金菊牛蒡茶(15)\n+ 黃耆元氣茶(5)", "玫瑰決明茶(14)\n+ 當歸紅棗茶(6)")
        eng = f"🌿 杯蓋刻字：{name}" if price_val == 1980 else "🌿 熟客回購加贈茶包禮"
    else:
        m_v, a_v = ("黃耆元氣茶(4)\n+ 金菊牛蒡茶(1)", "當歸紅棗茶(3)\n+ 黑豆漢方茶(2)") if "晨光" in ans[0] else ("洛神山楂茶(3)\n+ 金菊牛蒡茶(2)", "玫瑰決明茶(3)\n+ 黑豆漢方茶(2)") if "微風" in ans[0] else ("金菊牛蒡茶(4)\n+ 黃耆元氣茶(1)", "玫瑰決明茶(4)\n+ 當歸紅棗茶(1)")
        eng = "🌿 方案：一週輕體驗組"
    
    show_leaves()
    # 組合名稱 bundle 本身就帶有全名與價格 (例如: 首購限定組合 $1,980)
    msg = f"Hi 米寶！🐢✨\n預約：{bundle}\n我是：【{dg}】\n\n☀️ 晨曦：\n{m_v}\n\n🌙 午後：\n{a_v}\n\n{eng}\n期待這份草本溫暖。🌿🍵"
