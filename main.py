...
# 1. 視覺鎖定 (極致壓縮空間、去黑底、三色網底、落葉特效)
st.set_page_config(page_title="米寶漢方｜你的植感陪伴", layout="centered")
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光...", "我有些沉重，想找回輕盈自在的微風...", "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v42_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，循環緩慢且手腳冰冷", "工作忙碌常熬夜，晚餐不規律導致不適", "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v42_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v42_q3", label_visibility="collapsed")
    ...
    choice = st.radio("", opts, index=None, key="v42_choice", label_visibility="collapsed")
    ...
    st.markdown("""<div style='background-color: #FFFFFF; padding: 12px; border-radius: 10px; border: 1px solid #E9EDC9; font-size:0.85rem; text-align:center;'>為你準備一只質感的玻璃隨行杯。讓我們在木蓋上，鐫刻專屬的名字，陪伴你每一天。</div>""", unsafe_allow_html=True)
    ...
    <div style="background-color: #FFFFFF; padding: 12px; border-radius: 15px; border: 1px solid #E9EDC9;">
        <h3 style='margin:0; font-size:1.2rem !important;'>✨ 你是：{diag}氣質</h3>
...
# 【極簡定稿頁尾】
st.markdown("""
    <p style='text-align:center; font-size:0.7rem; color:#8B8B7A; margin-top:15px; line-height:1.4;'>
        米寶漢方｜慶和蔘藥行研製<br>
        本產品屬一般食品。 © 2026 Mibao Herbal
    </p>
    """, unsafe_allow_html=True)
