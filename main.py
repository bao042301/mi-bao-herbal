...
# 1. 究極視覺與跳轉鎖定 (一屏全覽、選項三色、選中加粗、Line連動)
...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    .block-container { padding-top: 0.1rem !important; padding-bottom: 60px !important; }

    /* 標題與題目縮小：一屏全覽的關鍵 */
    h3 { font-size: 0.85rem !important; font-weight: 700 !important; margin-top: -10px !important; margin-bottom: 2px !important; text-align: center !important; color: #7A8450 !important; }
    .question-text { font-size: 0.85rem !important; font-weight: bold !important; text-align: center !important; margin-bottom: 4px !important; line-height: 1.2 !important; }
    .quote { font-style: italic; color: #8B8B7A !important; text-align: center; margin-bottom: 5px !important; font-size: 0.65rem !important; }
    ...
    [data-testid="stRadio"] label {
        border-radius: 8px !important; padding: 8px 15px !important;
        width: 100% !important; border: 1px solid rgba(0,0,0,0.05) !important;
        display: flex !important; justify-content: flex-start !important; 
        text-align: left !important; font-size: 0.8rem !important; line-height: 1.3 !important;
    }
    ...
    .price-text { font-size: 1.3rem !important; font-weight: bold !important; color: #7A8450 !important; margin-top: 5px; display: block; text-align: center !important; }

    /* 徹底去黑底與複製按鈕強顯 */
    [data-testid="stCodeBlock"], [data-testid="stCodeBlock"] > div, pre, code { background-color: #F8F9F1 !important; border: 1px solid #E9EDC9 !important; border-radius: 8px !important; }
    [data-testid="stCodeBlock"] button { opacity: 1 !important; visibility: visible !important; display: block !important; background-color: rgba(233, 237, 201, 1) !important; border: 1px solid #7A8450 !important; scale: 0.7; }
    ...
    /* 按鈕樣式 */
    .stButton > button { width: 100% !important; background-color: #7A8450 !important; color: #FFFFFF !important; border-radius: 20px !important; height: 2.5em !important; font-weight: bold !important; }
    .stButton > button p { color: #FFFFFF !important; font-size: 0.85rem !important; }

    /* LINE 按鈕轉址增強 */
    .line-link { display: block !important; text-decoration: none !important; margin-top: 5px !important; position: relative !important; z-index: 9999 !important; }
    .line-btn-styled { background-color: #06C755 !important; color: white !important; text-align: center !important; padding: 10px; border-radius: 12px; font-weight: bold; font-size: 0.85rem; }

    /* 固定頁尾 */
    .custom-footer { position: fixed; left: 0; bottom: 8px; width: 100%; text-align: center; background-color: transparent; z-index: 99; }
...
# ----------------- 【核心優化】整合方案頁面 (Step 4) -----------------
    ...
    bundles = [
        "月度首購組 $1,980 [贈：精品刻名隨行杯 👋]",
        "月度回購組 $1,980 [贈：驚喜茶包 3 入 🤗]",
        "一週輕體驗組 $680 [初次邂逅草本 🍵]"
    ]
    selected = st.radio("", bundles, index=None, key="bundle_select", label_visibility="collapsed")
    
    # 動態輸入框：只有選首購才出現
    engraving_name = ""
    if selected == bundles[0]:
        st.markdown("<div style='background-color:#FFF; padding:8px; border-radius:8px; border:1px solid #E9EDC9; margin-bottom:5px;'><p style='font-size:0.75rem; margin:0;'>精品杯蓋雷刻文字 (12字內)：</p></div>", unsafe_allow_html=True)
        engraving_name = st.text_input("", placeholder="例如：Mila", label_visibility="collapsed")
    
    if st.button("查看我的專屬配比 ➔"):
        if selected:
            st.session_state.bundle = selected
            st.session_state.custom_name = engraving_name if selected == bundles[0] else "熟客驚喜"
...
# ----------------- 結果呈現 (Step 5) -----------------
    ...
    is_monthly = "$1,980" in bundle
    
    if is_monthly:
        pr, am = "$1,980", "40 入深度植感漢方茶組"
        ...
        gift = f"• 首購限定：專屬刻名隨行杯 ({name})" if "首購" in bundle else "• 老朋友禮：加贈驚喜茶包 3 包"
    else:
    ...
    st.markdown(f"""<div style="background-color: #FFFFFF; padding: 10px; border-radius: 12px; border: 1px solid #E9EDC9;">
        <h3 style='margin:0; font-size:1.1rem !important;'>✨ 您是：{dg}氣質</h3>
        ...
        <span class="price-text">專屬預約價 {pr}</span></div>""", unsafe_allow_html=True)
    if st.button("預約這份時光 ➔"): st.session_state.step = 6; st.rerun()
...
# ----------------- 預約轉化 (Step 6) -----------------
    ...
    if "$1,980" in bundle:
        ...
        eng = f"🌿 杯蓋刻字：{name}" if "首購" in bundle else "🌿 熟客月度回購禮"
    else:
    ...
    show_leaves()
    msg = f"Hi 米寶！🐢✨\n預約組合：{bundle}\n我是：【{dg}】\n☀️ 晨：{m_v}\n🌙 午：{a_v}\n{eng}\n期待這份草本溫暖。🌿🍵"
    st.markdown('<p style="font-size:0.75rem; text-align:center; margin-bottom:2px;">點擊☆右上角☆複製後貼給米寶：</p>', unsafe_allow_html=True)
