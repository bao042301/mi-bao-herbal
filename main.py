...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，想沐浴在溫潤透亮的晨光", "我有些沉重，想沉浸在輕盈自在的微風", "我有些燥熱，想念山間清澈甘甜的泉水"], index=None, key="v78_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，手腳冰冷循環差", "生活忙碌常熬夜，作息不規律隨意吃", "壓力大節奏快，心神緊繃難入眠"], index=None, key="v78_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，從容自在"], index=None, key="v78_q3", label_visibility="collapsed")
    ...
    opts = ["是的，我是新朋友 👋", "我是老朋友 🤗"]
    choice = st.radio("", opts, index=None, key="v78_choice", label_visibility="collapsed")
    ...
    choice = st.radio("", plan_opts, index=None, key="v78_plan", label_visibility="collapsed")
    ...
    st.markdown("""<div style='background-color: #FFFFFF; padding: 10px; border-radius: 12px; border: 1px solid #E9EDC9; font-size:0.8rem; text-align:center;'>
        我們將在木蓋上，鐫刻您的專屬風格。
    ...
    if plan == "月度":
        ...
        gt = f"• 首購禮：專屬刻名隨行杯 ({name})" if first=="是的" else "• 老朋友禮：加贈驚喜茶包 3 包"
    else:
    ...
    <div style="background-color: #FFFFFF; padding: 8px; border-radius: 10px; border: 1px solid #E9EDC9;">
        ...
        <p style='font-size:0.85rem; margin:0; font-weight:bold;'>☀️ 晨曦：{m_t}</p>
        <p style='font-size:0.85rem; margin:0; font-weight:bold;'>🌙 午後：{a_t}</p>
        <hr style='border: 0.5px solid #E9EDC9; margin: 4px 0;'>
        <p class="benefit-item" style='margin:0;'>• {am}<br>{gt}</p>
        <span class="price-text">方案陪伴價 {pr}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    dg = "暖陽系" if "晨光" in ans[0] else "微風系" if "微風" in ans[0] else "清泉系"
    if plan == "月度":
        m_v, a_v = ("黃耆(14)+金菊(6)", "當歸(12)+黑豆(8)") if "晨光" in ans[0] else ("洛神(12)+金菊(8)", "玫瑰(10)+黑豆(10)") if "微風" in ans[0] else ("金菊(15)+黃耆(5)", "玫瑰(14)+當歸(6)")
        eng = f"🌿 杯蓋刻上：{name}" if first == "是的" else "🌿 老朋友月度贈茶"
    else:
        m_v, a_v = ("黃耆(4)+金菊(1)", "當歸(3)+黑豆(2)") if "晨光" in ans[0] else ("洛神(3)+金菊(2)", "玫瑰(3)+黑豆(2)") if "微風" in ans[0] else ("金菊(4)+黃耆(1)", "玫瑰(4)+當歸(1)")
        eng = "🌿 一週輕體驗組"
    show_leaves()
    msg = f"Hi 米寶！🐢✨\n預約【{plan}】|【{dg}】\n☀️ 晨：{m_v}\n🌙 午：{a_v}\n{eng}\n期待與草本暖茶相遇。🌿🍵"
