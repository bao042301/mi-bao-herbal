...
# 1. 視覺與顏色鎖死 (全面鎖定米寶色調)
st.set_page_config(page_title="米寶漢方｜妳的月度植感練習", layout="centered")
...
st.markdown("""
    ...
    * {
    ...
    /* 2. 【關鍵修正】移除黑底區塊，改為信箋風格 */
    .order-box {
        background-color: #F8F9F1 !important; /* 淺草色背景 */
    ...
    .tea-tag { background-color: #FFE8D6 !important; color: #6B705C !important; padding: 2px 10px; border-radius: 20px; font-size: 0.85rem; font-weight: bold; float: right; }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("當妳靜下心，妳的身體正低聲說著...？", 
                  ["我有些疲累，渴望溫潤透亮的晨光...", 
                   "我有些沉重，想找回輕盈自在的微風...", 
                   "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v3_q1")
    ...
    q2 = st.radio("關於這段日子的作息，妳目前的感受是？", 
                  ["長時間待在冷氣房，循環緩慢且手腳冰冷", 
                   "工作忙碌常熬夜，晚餐不規律導致不適", 
                   "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v3_q2")
    ...
    q3 = st.radio("在最需要喘息的午後，妳最嚮往什麼樣的瞬間？", 
                  ["感覺臉龐恢復紅潤元氣，重新出發", 
                   "感覺身體找回輕盈律動，不再束縛", 
                   "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v3_q3")
    ...
    # 精確邏輯
    if "晨光" in ans[0]:
        diag, tea1, tea2 = "暖陽系", "黃耆元氣茶 (14入) + 金菊牛蒡茶 (6入)", "當歸紅棗茶 (12入) + 黑豆漢方茶 (8入)"
    elif "微風" in ans[0]:
        diag, tea1, tea2 = "微風系", "洛神山楂茶 (12入) + 金菊牛蒡茶 (8入)", "玫瑰決明茶 (10入) + 黑豆漢方茶 (10入)"
    else:
        diag, tea1, tea2 = "清泉系", "金菊牛蒡茶 (15入) + 黃耆元氣茶 (5入)", "玫瑰決明茶 (14入) + 當歸紅棗茶 (6入)"
    ...
    <div class="result-card">
        ...
        <p class="quote">「這份陪伴計畫，是送給妳的時光禮物。」</p>
        <hr style='border: 0.5px solid #E9EDC9;'>
        <p style='margin-bottom:5px;'><b>☀️ 晨曦啟幕配比：</b></p>
        <p style='font-size:0.95rem; line-height:1.6;'>{tea1}</p>
        <br>
        <p style='margin-bottom:5px;'><b>🌙 午後拾光配比：</b></p>
        <p style='font-size:0.95rem; line-height:1.6;'>{tea2}</p>
        <hr style='border: 0.5px solid #E9EDC9;'>
        <p style='font-size:0.9rem;'><b>內容包含：</b> 40入茶組 + 生活手札 + 雷刻刻名杯 (刻名：{name})</p>
        <h3 style='text-align:right; color:#6B705C !important;'>月度陪伴價 $1,980</h3>
    </div>
    """, unsafe_allow_html=True)
    
    # 【關鍵修正：取代黑底區塊】
    st.write("👇 請長按複製下方訊息，開啟妳的溫柔練習：")
