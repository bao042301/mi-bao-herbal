...
# 1. 究極視覺鎖定 (選中底色強勢覆蓋、價格置中放大、複製按鈕強顯、選項靠左)
...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* 【究極修正】當選項被選中後，使用最高權重路徑鎖定，強制改變所有層級底色 */
    /* 這裡同時針對 aria-checked 與 data-checked 進行暴力鎖定 */
    [data-testid="stRadio"] div[role="radiogroup"] div[aria-checked="true"],
    [data-testid="stRadio"] div[role="radiogroup"] div[aria-checked="true"] label,
    [data-testid="stRadio"] div[role="radiogroup"] div[data-checked="true"] label {
        background-color: #E9EDC9 !important; /* 強制切換為品牌橄欖綠 */
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光", "我有些沉重，想找回輕盈自在的微風", "我有些燥熱，想念山間清徹甘甜的泉水"], index=None, key="v68_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，手腳冰冷循環差", "生活忙碌常熬夜，作息不規規律隨意吃", "壓力大節奏快，心神緊繃難入眠"], index=None, key="v68_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，從容自在"], index=None, key="v68_q3", label_visibility="collapsed")
    ...
    opts = ["是的，我是新朋友", "我是老朋友"]
    choice = st.radio("", opts, index=None, key="v68_choice", label_visibility="collapsed")
