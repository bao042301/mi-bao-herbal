...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* 縮小 Logo 確保一屏全覽 */
    [data-testid="stImage"] img { max-height: 100px !important; width: auto !important; margin: 0 auto; display: block; }
    ...
    .quote { font-style: italic; color: #8B8B7A !important; text-align: center; margin-bottom: 25px; font-size: 0.85rem; }

    /* 選項卡片設計：移除黑點 + 三色網底 */
    div[data-baseweb="radio"] input { display: none !important; }
    div[data-baseweb="radio"] { gap: 15px !important; }
    ...
    /* 三色設定 */
    div[data-baseweb="radio"] > div:nth-child(1) label { background-color: #F1F4E8 !important; } /* 淡葉綠 */
    div[data-baseweb="radio"] > div:nth-child(2) label { background-color: #FDF2E9 !important; } /* 暖陽杏 */
    div[data-baseweb="radio"] > div:nth-child(3) label { background-color: #EBF5FB !important; } /* 清泉藍 */

    /* 選中狀態 */
    div[aria-checked="true"] { border: 2px solid #7A8450 !important; transform: scale(1.02) !important; }
    div[aria-checked="true"] * { font-weight: bold !important; }

    /* 按鈕樣式 */
    .stButton > button {
        width: 100% !important; background-color: #7A8450 !important; color: #FFFFFF !important;
        border-radius: 40px !important; height: 3.2em !important; font-weight: bold !important; border: none !important;
    }
    ...
    .stCodeBlock, pre, code, div[data-testid="stCodeBlock"], div[data-testid="stCodeBlock"] > div {
        background-color: #F8F9F1 !important; border: 1px solid #E9EDC9 !important; border-radius: 15px !important;
    }
    ...
    .leaf { position: fixed; top: -10vh; font-size: 24px; pointer-events: none; z-index: 9999; animation: falling 10s linear infinite; }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光...", "我有些沉重，想找回輕盈自在的微風...", "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v25_q1")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，循環緩慢且手腳冰冷", "工作忙碌常熬夜，晚餐不規律導致不適", "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v25_q2")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v25_q3")
    ...
    choice = st.radio("", ["是的，我是新朋友", "不是，我是老朋友了（已有隨行杯）"], index=None, key="v25_choice")
    ...
    st.markdown("<div style='background-color: #FFFFFF; padding: 20px; border-radius: 15px; border: 1px solid #E9EDC9; font-size:0.9rem;'>首購禮遇：為妳在隨行杯木蓋上，鐫刻專屬的名字。</div>", unsafe_allow_html=True)
    ...
    show_leaves() # 落葉特效回歸
    gift_detail = f"• <b>精品首購禮：</b>專屬刻名隨行杯 (刻名：{name})" if first == "是的" else "• <b>老友回饋禮：</b>額外加贈 3 包驚喜茶包"
    ...
    <div class="result-card">
        ...
        <p style='font-size:0.95rem; margin-bottom:5px;'>☀️ 晨曦：{m_tea}</p>
        <p style='font-size:0.95rem; margin-bottom:5px;'>🌙 午後：{a_tea}</p>
        <hr style='border: 0.5px solid #E9EDC9; margin: 10px 0;'>
        ...
        <p style='text-align:right; color:#7A8450; font-size: 1rem; font-weight: bold;'>月度陪伴價 $1,980</p>
    </div>
    """, unsafe_allow_html=True)
...
st.markdown("<p style='text-align:center; font-size:0.75rem; color:#8B8B7A; margin-top:30px;'>米寶漢方｜慶和蔘藥行研製<br>產品為一般食品。 © 2026 Mibao Herbal</p>", unsafe_allow_html=True)
