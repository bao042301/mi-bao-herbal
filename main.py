...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* Logo 尺寸 */
    [data-testid="stImage"] img { max-height: 80px !important; width: auto !important; margin: 0 auto !important; display: block; }
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，渴望溫潤透亮的晨光...", "我有些沉重，想找回輕盈自在的微風...", "我有些燥熱，想念山間清徹甘甜的泉水..."], index=None, key="v38_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，循環緩慢且手腳冰冷", "工作忙碌常熬夜，晚餐不規律導致不適", "壓力大節奏快，晚上難以入眠且心神不寧"], index=None, key="v38_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，優雅從容"], index=None, key="v38_q3", label_visibility="collapsed")
    ...
    choice = st.radio("", opts, index=None, key="v38_choice", label_visibility="collapsed")
    ...
    # 【增加儀式感文案】
    st.markdown("""
        <div style='background-color: #FFFFFF; padding: 20px; border-radius: 15px; border: 1px solid #E9EDC9; margin-bottom: 20px;'>
            「為妳準備一只質感的玻璃隨行杯。讓這只杯子陪妳慢下來，每一次舉杯，都提醒妳值得被溫柔對待。」<br><br>
            <b>首購禮遇：讓我們在木蓋上，鐫刻專屬的名字。</b>
        </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="background-color: #FFFFFF; padding: 18px; border-radius: 20px; border: 1px solid #E9EDC9;">
    ...
    st.write("### 📢 預約暖心的相遇")
    # 【增加引導溫度文案】
    st.write("最後一步，完成暖心的約定。點擊下方框框右上角的圖示複製文字，交給米寶店長，讓我們為妳揉捻這份草本香氣：")
...
st.markdown("<p style='text-align:center; font-size:0.75rem; color:#8B8B7A; margin-top:20px;'>米寶漢方｜慶和蔘藥行研製 © 2026 Mibao Herbal</p>", unsafe_allow_html=True)
