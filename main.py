...
# 1. 究極視覺鎖定 (選中字體加粗放大、價格置中放大、複製按鈕強顯、選項靠左)
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，想沐浴在溫潤透亮的晨光", "我有些沉重，想沉浸在輕盈自在的微風", "我有些燥熱，想念山間清澈甘甜的泉水"], index=None, key="v75_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，手腳冰冷循環差", "生活忙碌常熬夜，作息不規律隨意吃", "壓力大節奏快，心神緊繃難入眠"], index=None, key="v75_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，從容自在"], index=None, key="v75_q3", label_visibility="collapsed")
    ...
    opts = ["是的，我是新朋友 👋", "我是老朋友 🤗"]
    choice = st.radio("", opts, index=None, key="v75_choice", label_visibility="collapsed")
    ...
    plan_opts = ["月度植感陪伴 (40入/全月份量) $1,680", "一週輕體驗組 (10入/初次嘗試) $680"]
    choice = st.radio("", plan_opts, index=None, key="v75_plan", label_visibility="collapsed")
    ...
    <div style="background-color: #FFFFFF; padding: 12px; border-radius: 15px; border: 1px solid #E9EDC9;">
        ...
        <p class="benefit-item" style='margin:0;'>
            • {amount}<br>• 米寶生活語錄收藏卡 (暖心支持)<br>{gift}<br>
        </p>
...
期待與這份草本暖茶相遇。🌿🍵"""
    ...
    st.markdown('<p style="font-size:0.9rem; margin-bottom:5px; text-align:center;"><br>點擊☆下方框框右上角☆複製文字：</p>', unsafe_allow_html=True)
    st.code(msg, language=None)
    
    st.markdown(f'<a href="https://line.me/R/ti/p/@716osfvq" style="text-decoration:none;"><div style="background-color: #06C755; color: white; text-align: center; padding: 12px; border-radius: 12px; font-weight: bold; margin-bottom:5px;">🌿 前往 LINE@貼上專屬配方給米寶吧！ ➔</div></a>', unsafe_allow_html=True)
