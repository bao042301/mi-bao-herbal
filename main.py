...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    /* 容器間距極致壓縮：減少 50% 垂直空間 */
    .block-container { padding-top: 0rem !important; padding-bottom: 20px !important; max-width: 95% !important; }
    ...
    .quote { font-style: italic; color: #8B8B7A !important; text-align: center; margin-bottom: 4px !important; font-size: 0.65rem !important; line-height: 1.1 !important; }

    /* Logo 尺寸固定 */
    [data-testid="stImage"] img { max-height: 35px !important; width: auto !important; margin: 0 auto !important; display: block; }
    [data-testid="stImage"] { margin-bottom: -10px !important; margin-top: -5px !important; }
    ...
    [data-testid="stRadio"] label {
        ...
        text-align: left !important; font-size: 0.78rem !important; line-height: 1.2 !important;
    }
    /* 黑點微縮 */
    ...
    /* 複製按鈕強顯 */
    [data-testid="stCodeBlock"] button { opacity: 1 !important; visibility: visible !important; background-color: #E9EDC9 !important; border: 1px solid #7A8450 !important; scale: 0.6; right: 5px !important; top: 5px !important; }
    code span { background-color: transparent !important; color: #4A4E31 !important; }

    /* 按鈕樣式：橄欖綠 */
    .stButton > button { width: 100% !important; background-color: #7A8450 !important; color: #FFFFFF !important; border-radius: 15px !important; height: 2.2em !important; font-weight: bold !important; border: none !important; }
    .stButton > button p { color: #FFFFFF !important; font-size: 0.8rem !important; }

    /* LINE 按鈕：強效連結包裝 */
    .line-cta-wrap { display: block !important; text-decoration: none !important; margin: 5px 0 !important; cursor: pointer !important; }
    .line-cta-body { background-color: #06C755 !important; color: white !important; text-align: center !important; padding: 10px; border-radius: 12px; font-weight: bold; font-size: 0.85rem; box-shadow: 0 3px 6px rgba(0,0,0,0.1); }
...
# ----------------- 結果頁面 (Step 5) -----------------
    ...
    if price_val >= 1880:
        ...
        gift = f"• 首購禮：精品隨行杯 ({name})" if price_val == 1980 else "• 老友禮：加贈茶包 3 包"
        amt = "40 入全月配比漢方茶"
    else:
    ...
    st.markdown(f"""<div style="background-color: #FFFFFF; padding: 10px; border-radius: 12px; border: 1px solid #E9EDC9;">
        ...
        <hr style='border: 0.5px solid #E9EDC9; margin: 4px 0;'>
        <p style='font-size:0.8rem; margin:0; font-weight:bold; color:#7A8450;'>• {amt}<br>{gift}</p>
        <span style="display:block; text-align:center; font-size:1.2rem; font-weight:900; color:#7A8450; margin-top:8px;">專屬預約價 ${price_val}</span></div>""", unsafe_allow_html=True)
...
# ----------------- 預約轉化 (Step 6) -----------------
    ...
    if price_val >= 1880:
        if "晨光" in ans[0]: m_v, a_v = "黃耆元氣茶(14)+金菊牛蒡茶(6)", "當歸紅棗茶(12)+黑豆漢方茶(8)"
        elif "微風" in ans[0]: m_v, a_v = "洛神山楂茶(12)+金菊牛蒡茶(8)", "玫瑰決明茶(10)+黑豆漢方茶(10)"
        else: m_v, a_v = "金菊牛蒡茶(15)+黃耆元氣茶(5)", "玫瑰決明茶(14)+當歸紅棗茶(6)"
        eng = f"🌿 杯蓋刻字：{name}" if price_val == 1980 else "🌿 熟客回購加贈禮包"
    else:
        if "晨光" in ans[0]: m_v, a_v = "黃耆元氣茶(4)+金菊牛蒡茶(1)", "當歸紅棗茶(3)+黑豆漢方茶(2)"
        elif "微風" in ans[0]: m_v, a_v = "洛神山楂茶(3)+金菊牛蒡茶(2)", "玫瑰決明茶(3)+黑豆漢方茶(2)"
        else: m_v, a_v = "金菊牛蒡茶(4)+黃耆元氣茶(1)", "玫瑰決明茶(4)+當歸紅棗茶(1)"
        eng = "🌿 方案：一週輕體驗組"
    ...
    # 【最強效跳轉連結】
    st.markdown(f'''<a href="https://line.me/R/ti/p/@716osfvq" target="_blank" class="line-cta-wrap"><div class="line-cta-body">🌿 前往 LINE@ 貼上配方預約米寶吧！ ➔</div></a>''', unsafe_allow_html=True)
