...
# 1. 究極視覺鎖定 (黑點縮小、LINE跳轉修復、極致一屏、字體加粗)
...
st.markdown("""
    ...
    * { color: #4A4E31 !important; font-family: 'Noto Sans TC', sans-serif !important; }
    ...
    .line-link {
        ...
        z-index: 9999 !important; /* 確保按鈕在最上層，不會點不到 */
    }
    .line-btn-styled {
...
if st.session_state.step == 1:
    ...
    q1 = st.radio("", ["我有些疲累，想沐浴在溫潤透亮的晨光", "我有些沉重，想沉浸在輕盈自在的微風", "我有些燥熱，想念山間清澈甘甜的泉水"], index=None, key="v76_q1", label_visibility="collapsed")
    ...
    q2 = st.radio("", ["長時間待在冷氣房，手腳冰冷循環差", "生活忙碌常熬夜，作息不規律隨意吃", "壓力大節奏快，心神緊繃難入眠"], index=None, key="v76_q2", label_visibility="collapsed")
    ...
    q3 = st.radio("", ["感覺臉龐恢復紅潤元氣，重新出發", "感覺身體找回輕盈律動，不再束縛", "感覺內心恢復安靜穩定，從容自在"], index=None, key="v76_q3", label_visibility="collapsed")
    ...
    opts = ["是的，我是新朋友 👋", "我是老朋友 🤗"]
    choice = st.radio("", opts, index=None, key="v76_choice", label_visibility="collapsed")
    ...
    choice = st.radio("", plan_opts, index=None, key="v76_plan", label_visibility="collapsed")
    ...
    if plan == "月度":
        ...
        if "晨光" in ans[0]: m_tea, a_tea = "黃耆(4)+金菊(1)", "當歸(3)+黑豆(2)"
        elif "微風" in ans[0]: m_tea, a_tea = "洛神(3)+金菊(2)", "玫瑰(3)+黑豆(2)"
        else: m_tea, a_tea = "金菊(4)+黃耆(1)", "玫瑰(4)+當歸(1)"
        gift = "• 體驗方案：初探草本植感相遇"
    show_leaves() 
    ...
    diag = "暖陽系" if "晨光" in ans[0] else "微風系" if "微風" in ans[0] else "清泉系"
    if plan == "月度":
        m_t, a_t = ("黃耆(14)+金菊(6)", "當歸(12)+黑豆(8)") if "晨光" in ans[0] else ("洛神(12)+金菊(8)", "玫瑰(10)+黑豆(10)") if "微風" in ans[0] else ("金菊(15)+黃耆(5)", "玫瑰(14)+當歸(6)")
        eng = f"🌿 杯蓋刻上：{name} ✨" if first == "是的" else "🌿 老朋友月度組，回購領贈茶"
    else:
        m_t, a_t = ("黃耆(4)+金菊(1)", "當歸(3)+黑豆(2)") if "晨光" in ans[0] else ("洛神(3)+金菊(2)", "玫瑰(3)+黑豆(2)") if "微風" in ans[0] else ("金菊(4)+黃耆(1)", "玫瑰(4)+當歸(1)")
        eng = "🌿 方案：一週輕體驗組 🍵"
    show_leaves()
    msg = f"Hi 米寶！🐢✨\n預約【{plan}方案】|【{diag}】\n☀️ 晨：{m_t}\n🌙 午：{a_t}\n{eng}\n期待相遇。🌿"
    st.markdown('<p style="font-size:0.8rem; margin-bottom:2px; text-align:center;">點擊☆右上角☆複製文字後傳給米寶：</p>', unsafe_allow_html=True)
    st.code(msg, language=None)
    # 【核心優化】加強轉址穩定性的 LINE 按鈕
